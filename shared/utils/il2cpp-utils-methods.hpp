...
namespace il2cpp_utils {
    ...
    TOut RunMethodThrow(T&& instance, const MethodInfo* method, TArgs&& ...params) {
        ...
        if (exp) {
            Logger::get().error("il2cpp_utils: RunMethodThrow: %s: Failed with exception: %s", il2cpp_functions::method_get_name(method),
    ...
    ::std::optional<TOut> RunMethod(T&& instance, const MethodInfo* method, TArgs&& ...params) {
        ...
        if (exp) {
            logger.error("il2cpp_utils: RunMethod: %s: Failed with exception: %s", il2cpp_functions::method_get_name(method),
                il2cpp_utils::ExceptionToString(exp).c_str());
            return ::std::nullopt;
        }
